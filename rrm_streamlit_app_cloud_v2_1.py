...
with tab_yield:
    ...
    # Overall by Paddy: (sum rice out by paddy) / (sum paddy used) * 100
    ...
    # Safe numeric conversion to avoid dtype issues
    ...
    # Daily yield (all paddies combined): rice_out / paddy_used for each date
    dy = df_read("""
        SELECT d.dt as Date,
               COALESCE(mu.used_qtl,0) as Paddy_Used_qtl,
               COALESCE(mo.out_qtl,0) as Rice_Out_qtl,
               CASE WHEN COALESCE(mu.used_qtl,0)=0 THEN NULL
                    ELSE ROUND(COALESCE(mo.out_qtl,0)*100.0/COALESCE(mu.used_qtl,1),2) END as Yield_%
        FROM (
            SELECT dt FROM milling_input
            UNION
            SELECT dt FROM milling_output
        ) d
        LEFT JOIN (SELECT dt, SUM(final_used_qtl) as used_qtl FROM milling_input GROUP BY dt) mu ON mu.dt=d.dt
        LEFT JOIN (SELECT dt, SUM(final_out_qtl) as out_qtl  FROM milling_output GROUP BY dt) mo ON mo.dt=d.dt
        WHERE d.dt IS NOT NULL AND d.dt<>''
        ORDER BY d.dt DESC
    """)
