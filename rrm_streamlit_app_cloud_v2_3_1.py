...
kg_per_qtl = st.sidebar.number_input("KG per Quintal", min_value=1, max_value=200, value=int(df_read("SELECT value FROM config WHERE key='kg_per_qtl'")["value"].astype(int).iloc[0]))
...
# Helper to layout forms in mobile mode
def cols(n):
    return (st.columns(1) if mobile_mode else st.columns(n))

# ----- Purchases -----
with tab_pur:
    st.subheader("🛒 Purchases")
    c1, c2 = cols(2)
    dfrom = c1[0].date_input("From", value=date(2025,1,1), key="pur_from") if mobile_mode else c1.date_input("From", value=date(2025,1,1), key="pur_from")
    dto   = c2[0].date_input("To", value=date.today(), key="pur_to")       if mobile_mode else c2.date_input("To", value=date.today(), key="pur_to")
    ...
    a = cols(4)
    dt_new = a[0].date_input("Date", value=date.today(), key="pur_dt2")
    pid = a[1].selectbox("Paddy", paddy_list["paddy_id"], format_func=lambda x: paddy_list.set_index("paddy_id").loc[x,"paddy_name"])
    qty = a[2].number_input("Qty (qtl)", min_value=0.0, step=0.1, key="pur_qty2")
    rate = a[3].number_input("Rate (₹/qtl)", min_value=0.0, step=50.0, key="pur_rate2")
    ...
    if not pdata_f.empty:
        ...
        e = cols(3)
        new_qty = e[0].number_input("Qty (qtl)", value=float(row["final_qtl"]), step=0.1, key="pur_edit_qty2")
        new_rate = e[1].number_input("Rate (₹/qtl)", value=float(row["rate_qtl"]), step=50.0, key="pur_edit_rate2")
        new_notes = e[2].text_input("Notes", value=row["notes"] or "", key="pur_edit_notes2")
...
with tab_mi:
    ...
    c1, c2 = cols(2)
    dfrom = c1[0].date_input("From ", value=date(2025,1,1), key="mi_from2") if mobile_mode else c1.date_input("From ", value=date(2025,1,1), key="mi_from2")
    dto   = c2[0].date_input("To ", value=date.today(), key="mi_to2")       if mobile_mode else c2.date_input("To ", value=date.today(), key="mi_to2")
    ...
    a = cols(3)
    mi_dt = a[0].date_input("Date", value=date.today(), key="mi_add_dt2")
    mi_pid = a[1].selectbox("Paddy", paddy_list2["paddy_id"], format_func=lambda x: paddy_list2.set_index("paddy_id").loc[x,"paddy_name"], key="mi_add_pid2")
    mi_used = a[2].number_input("Used (qtl)", min_value=0.0, step=0.1, key="mi_add_used2")
    b = cols(3)
    mi_husk = b[0].number_input("Husk (qtl)", min_value=0.0, step=0.1, key="mi_add_husk2")
    mi_pol  = b[1].number_input("Polish (qtl)", min_value=0.0, step=0.1, key="mi_add_pol2")
    mi_exp  = b[2].number_input("Expense (₹)", min_value=0.0, step=100.0, key="mi_add_exp2")
    ...
    if not tdf.empty:
        ...
        e = cols(4)
        used = e[0].number_input("Used (qtl)", value=float(row["final_used_qtl"]), step=0.1, key="mi_edit_used2")
        husk = e[1].number_input("Husk (qtl)", value=float(row["husk_qtl"] or 0), step=0.1, key="mi_edit_husk2")
        polish = e[2].number_input("Polish (qtl)", value=float(row["polish_qtl"] or 0), step=0.1, key="mi_edit_pol2")
        exp = e[3].number_input("Expense (₹)", value=float(row["expense"] or 0), step=100.0, key="mi_edit_exp2")
...
with tab_mo:
    ...
    c = cols(3)
    dfrom = c[0].date_input("From  ", value=date(2025,1,1), key="mo_from2") if mobile_mode else c[0].date_input("From  ", value=date(2025,1,1), key="mo_from2")
    dto   = c[1].date_input("To  ", value=date.today(), key="mo_to2")       if mobile_mode else c[1].date_input("To  ", value=date.today(), key="mo_to2")
    grade_filter = c[2].text_input("Filter by Grade (name contains)", key="mo_grade_f2")
    ...
    a = cols(3)
    mo_dt = a[0].date_input("Date", value=date.today(), key="mo_add_dt2")
    mo_pid = a[1].selectbox("Paddy", paddy_list3["paddy_id"], format_func=lambda x: paddy_list3.set_index("paddy_id").loc[x,"paddy_name"], key="mo_add_pid2")
    mo_gid = a[2].selectbox("Grade", grade_list3["grade_id"], format_func=lambda x: grade_list3.set_index("grade_id").loc[x,"grade_name"], key="mo_add_gid2")
    b = cols(2)
    mo_qty = b[0].number_input("Rice OUT (qtl)", min_value=0.0, step=0.1, key="mo_add_qty2")
    mo_notes = b[1].text_input("Notes", key="mo_add_notes2")
    ...
    if not mdf.empty:
        ...
        e = cols(2)
        qty = e[0].number_input("Out (qtl)", value=float(row["final_out_qtl"]), step=0.1, key="mo_edit_qty2")
        notes = e[1].text_input("Notes", value=row["notes"] or "", key="mo_edit_notes2")
...
with tab_sales:
    st.subheader("🧾 Sales")
    c = cols(3)
    dfrom = c[0].date_input("From   ", value=date(2025,1,1), key="sa_from2")
    dto   = c[1].date_input("To   ", value=date.today(), key="sa_to2")
    prod  = c[2].selectbox("Product filter", ["All","Rice","Husk","Polish"], key="sa_prod2")
    ...
    a = cols(3)
    sa_dt = a[0].date_input("Date", value=date.today(), key="sa_add_dt2")
    sa_prod = a[1].selectbox("Product", ["Rice","Husk","Polish"], key="sa_add_prod2")
    sa_gid = a[2].selectbox("Grade (if Rice)", options=[""] + grade_list4["grade_id"].tolist(), index=0,
                             format_func=lambda x: (grade_list4.set_index("grade_id").loc[x,"grade_name"] if x else ""), key="sa_add_gid2")
    b = cols(3)
    sa_qty = b[0].number_input("Qty (qtl)", min_value=0.0, step=0.1, key="sa_add_qty2")
    # default price
    def_rate = 0.0
    try:
        ...
        def_rate = 0.0
    sa_rate = b[1].number_input("Rate (₹/qtl)", min_value=0.0, step=50.0, value=def_rate, key="sa_add_rate2")
    sa_notes = b[2].text_input("Notes", key="sa_add_notes2")
    ...
    if not sdf.empty:
        ...
        e = cols(3)
        qty = e[0].number_input("Qty (qtl)", value=float(row["final_qtl"]), step=0.1, key="sa_edit_qty2")
        rate = e[1].number_input("Rate (₹/qtl)", value=float(row["rate_qtl"] or 0), step=50.0, key="sa_edit_rate2")
        notes = e[2].text_input("Notes", value=row["notes"] or "", key="sa_edit_notes2")
